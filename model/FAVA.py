...
def _get_offset_mapping_list(text_list, tokenizer):
    tokenized_inputs = tokenizer(
        text_list, return_offsets_mapping=True, add_special_tokens=False
    )
...
def predict_hallucinations(
    ...
    evidence = "\n".join(evidence_list)
    prompts = [
        INPUT_PROMPT_TEMPLATE.format_map(
            {"evidence": evidence, "output": hallucinated_output}
        )
    ]
    ...
    assert (
        len(token_ids_list)
        == len(logprobs_list)
        == len(FAVA_output_text_list)
        == len(offset_mapping_list)
        == 1
    )
    ...
    for start_idx, end_idx, hallucinated_text in hallucination_spans:
        ...
        try:
            assert (
                token_start_idx is not None and token_end_idx is not None
            ), f"Cannot find the token indices for the hallucinated span. Start: {start_idx}, End: {end_idx}, Hallucinated text: {hallucinated_text}"
        ...
        try:
            ...
            print(
                f"IndexError: token_start_idx: {token_start_idx}, token_end_idx: {token_end_idx}, len(prob_list): {len(prob_list)}"
            )
            exit()
...
if __name__ == "__main__":
    ...
    sampling_params = vllm.SamplingParams(
        temperature=0, top_p=1.0, max_tokens=1024, logprobs=1
    )
    ...
    if not os.path.exists(
        os.path.join(
            ...
            os.path.basename(args.yaml_filepath.replace(".yaml", "")),
        )
    ):
        os.makedirs(
            os.path.join(
    ...
    output_filepath = os.path.join(
        args.output_directory,
        os.path.basename(args.yaml_filepath.replace(".yaml", "")),
        f"{predictions[0]['lang'].lower()}_FAVA.jsonl",
    )
    ...
    try:
        ...
        print(
            'type(predictions[0]["hard_labels"]):', type(predictions[0]["hard_labels"])
        )
        print(
            'type(predictions[0]["soft_labels"]):', type(predictions[0]["soft_labels"])
        )
