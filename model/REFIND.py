    ...
    for idx in range(len(model_id_list)):
        ...
        for record in tqdm(records, desc="Processing records"):
            ...
            try:
                ...
                start_idx = offsets_mapping[-1][1]
                end_idx = len(model_output_text)

                offsets_mapping.append((start_idx, end_idx))
    ...
    for condition in HALLUCINATION_CONDITIONS.keys():
        for threshold in threshold_list:
            ...
            for prediction in predictions:
                ...
                # assert len(hard_labels) == len(soft_labels), f"Hard and soft labels mismatch! {len(hard_labels)} vs {len(soft_labels)}. Prediction: {prediction}"
