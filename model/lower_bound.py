...
def main(input_file, output_file, threshold):
    ...
    for record in tqdm(records, desc="Processing records"):
        ...
        idx = 0
        while (
            start_span < record["text_len"]
            and idx < len(record["model_output_tokens"]) - 1
        ):
            ...
            if token_prob < threshold:
                ...
                hallucination_span_ranges_with_probs.append(
                    {"start": start_span, "end": end_span, "prob": confidence}
                )
        ...
        if len(hallucination_span_ranges_with_probs) > 0:
            merged_hallucination_span_ranges_with_probs = [
                hallucination_span_ranges_with_probs[0]
            ]
            for span in hallucination_span_ranges_with_probs[1:]:
                if (
                    span["start"]
                    == merged_hallucination_span_ranges_with_probs[-1]["end"]
                ):
                    merged_hallucination_span_ranges_with_probs[-1]["end"] = span["end"]
                    # merge probabilities
                    merged_hallucination_span_ranges_with_probs[-1]["prob"] = max(
                        merged_hallucination_span_ranges_with_probs[-1]["prob"],
                        span["prob"],
                    )
                else:
