...
class Index_Builder:
    ...
    def __init__(
        ...
        if self.instruction is not None:
            ...
            if self.instruction == "":
                warnings.warn(f"Instruction is not set!")
            else:
    ...
    def st_encode_all(self):
        ...
        sentence_list = [item["title"] for item in self.corpus]
        sentence_list = [f"{self.instruction}{doc}" for doc in sentence_list]
        all_embeddings = self.encoder.encode(sentence_list, batch_size=self.batch_size, is_query=False)
    ...
    def encode_all(self):
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc="Inference Embeddings:"):
            batch_data = self.corpus[start_idx : start_idx + self.batch_size]["title"]
    ...
    def build_dense_index(self):
        ...
        print(f"Index type: {self.faiss_type}")
        ...
        if self.faiss_gpu:
            ...
            print("Index will be created on GPU")
            faiss_index = faiss.index_cpu_to_all_gpus(faiss_index, co)
            print("Index created on GPU")

            if not faiss_index.is_trained:
                print("Training index on GPU")
                faiss_index.train(all_embeddings)
                print("Index trained on GPU")
            ...
            print("Index moved to CPU")
        else:
            if not faiss_index.is_trained:
                faiss_index.train(all_embeddings)
            faiss_index.add(all_embeddings)

        print(f"Index will be saved to {self.index_save_path}")
